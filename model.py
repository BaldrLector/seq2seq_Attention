...
def lstm(inputs, num_units=None, bidirection=False, scope="lstm", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if bidirection:
            ...
            return tf.concat(outputs, 2), cell, cell_bw
        else:
            outputs, _ = tf.nn.dynamic_rnn(cell, inputs, dtype=tf.float32)
            return outputs
...
def attention_decoder(inputs, memory, num_units=None, scope="attention_decoder", reuse=None):
    ...
    return outputs, state, attention_mechanism, decoder_cell, cell_with_attention
...
def prenet(inputs, num_units=None, is_training=True, scope="prenet", reuse=None):
    ...
    return outputs  # (N, ..., num_units[1]) or (N,..,hp.embed_size//2)
